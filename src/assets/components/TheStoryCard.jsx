...
const TheStoryCard = ({ userName = "Dhina", id = 22 }) => {
  ...
  const storyNavigation = (id) => {
    navigate(`/story/${id}`); // navigates using the hash-aware router
  };

  return (
    <div
      className="flex-col justify-items-center items-center w-20"
      onClick={() => storyNavigation(id)}
    >
